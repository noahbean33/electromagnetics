...
// \(Loss = \frac{\sigma \Delta t}{2 \epsilon}\)
// \(C_E^E = \frac{1-Loss}{1+Loss}\)
// \(C_E^H = \frac{\frac{\eta_0}{\epsilon_r}S_c}{1+Loss}\)
typedef struct {
	PetscScalar e, h;
	PetscScalar cee, ceh;
} FieldNode;
...
#define E(x)                                                                                                           \
	...
	E(DMDACreate1d(PETSC_COMM_WORLD, DM_BOUNDARY_GHOSTED, N, 4, 1, NULL, &domain));
	E(DMSetUp(domain));
	const char* const names[] = {"e", "h", "cee", "ceh"};
	E(DMDASetFieldNames(domain, names));
