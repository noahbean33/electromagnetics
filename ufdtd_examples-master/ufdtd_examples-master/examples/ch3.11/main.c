...
// instead of separate E and H arrays using array of structs holding both values and relative permittivity
// there're 3 values in one node, so DOF is 3
typedef struct {
	PetscScalar e, h;
	PetscScalar eps;
} FieldNode;
...
#define E(x)                                                                                                           \
	...
	DM domain;
	// We now have 3 degrees of freedom
	// Note that we access our vectors in a same manner as for 2 DOF case
	// This is convenient because we can introduce new node properties without touching the update loop
	E(DMDACreate1d(PETSC_COMM_WORLD, DM_BOUNDARY_GHOSTED, N, 3, 1, NULL, &domain));
	E(DMSetUp(domain));
	E(DMDASetFieldName(domain, 0, "e"));
	E(DMDASetFieldName(domain, 1, "h"));
	E(DMDASetFieldName(domain, 2, "epsilon"));
	...
	for (PetscInt t = 0; t < Tmax; t++) {
		...
		for (PetscInt i = start + 1; i < end-1; i++) {
			...
			if (i == 0) {
				...
				farr[i].e += (farr[i].h - farr[i - 1].h) * imp0 / farr[i].eps;
			}
			if (i == SN+1) {
